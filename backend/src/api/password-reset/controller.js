...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your pm-backend account.<br>
        ...
        &mdash; pm-backend Team
      `
      return sendMail({ toEmail: email, subject: 'pm-backend - Password Reset', content })
    })
